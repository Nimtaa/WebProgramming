$(function() {
    $.ajax({
        ...
        success: function(data) {
            ...
            $('.more_foods').css('margin-left','215px');
            $('.more_foods').css('margin-right','215px');
...
function RenderSection5_Grid_Container(data){
    ...
    $(".section5_grid_container").children().each(function(){
        ...
        $(this).children("#sec5_input").css('background-size', "cover");
        if(index==3){
            $(this).children("#sec5_input").css('background-blend-mode', "darken");
        }
        index--;
    });
}
